...
export function ScriptureQuote({
  ...
  return (
    <blockquote
      className={cn('relative border p-6 bg-muted/20 rounded-lg', compact && 'p-4', className)}
    >
      <p className={cn('italic', compact ? 'text-sm' : 'text-base md:text-lg')}>
        {!text.startsWith('"') && !text.startsWith('„') ? '"' : ''}
        {text}
        {!text.endsWith('"') && !text.endsWith('"') ? '"' : ''}
      </p>
      <footer className="text-sm text-muted-foreground mt-2 font-medium">— {reference}</footer>
